...
-(void)createConnection:(NSString *) email description:(NSString *)description firstName:(NSString *)firstName lastName:(NSString *)lastName school:(NSString *)school image:(UIImage *) image
{
    ...
    NSString *myParameters = [NSString stringWithFormat: @"email=%@&description=%@&firstName=%@&lastName=%@&school=%@&image=%@",email,description,firstName,lastName,school,image];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if ([receivedData length] != 0)
    {
        ...
        {
            [delegate isEditProfileSuccessful:NO handleError:errorString];
            result = nil;
            jsonObject = nil;
        }
        ...
        else if([result caseInsensitiveCompare:@"passed"] == NSOrderedSame)
        {
            ...
            [delegate isEditProfileSuccessful:YES handleError:@""];
            result = nil;
            jsonObject = nil;
        }
