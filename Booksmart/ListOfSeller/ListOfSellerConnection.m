...
@implementation ListOfSellerConnection
...
-(void)createConnection:(NSString *) title
{
    NSString* link = [NSString stringWithFormat:@"%@%@", [WTTSingleton sharedManager].serverURL, @"/include_php/getListOfSeller.php"];
    NSMutableURLRequest *theRequest=[NSMutableURLRequest
                                     requestWithURL:[NSURL URLWithString: link]
                                     cachePolicy:NSURLRequestUseProtocolCachePolicy
                                     timeoutInterval:15.0];
    NSString *myParameters = [NSString stringWithFormat: @"title=%@",title];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if ([receivedData length] != 0)
    {
        ...
        else if([result caseInsensitiveCompare:@"passed"] == NSOrderedSame)
        {
            ...
            [delegate finishedListOfSellerConnection];
